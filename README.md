...
1. Pré-requisitos
Python 3.9+
Conta na API-FOOTBALL (existe plano gratuito limitado)
Bot do Telegram criado via @BotFather
2. Clonar e instalar dependências
```bash
git clone https://github.com/gmagusteiro/OpuSports.git
cd OpuSports
python3 -m venv venv
...
Recolhe histórico real: corre `data_collector.py` periodicamente (ou
...
Regista o resultado real de cada evento (ex.: se houve mais de 9.5
...
do jogo terminar.
Substitui `generate_synthetic_dataset()`: cria uma função equivalente
que faça `SELECT` às tuas tabelas reais e construa um `DataFrame` com as
mesmas colunas em `FEATURE_COLUMNS` + as colunas `target_*`.
Chama `train_all_models(df=o_teu_dataframe)` em vez de deixar `df=None`.
Valida sempre com validação cruzada (já implementada) e desconfia de
...
próprio jogo).
Considera recalibrar o modelo periodicamente (os dados de futebol mudam
...
recreativa/controlada, procura apoio (em Portugal: SICAD
